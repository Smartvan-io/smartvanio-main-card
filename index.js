      ...
      <div class="lp-panel">
        ...
        <div class="lp-list">
          ${s.map(({eid:t,state:e,_slotName:s},r)=>{const a="on"===e?.state,n=!e||"unavailable"===e.state||"unknown"===e.state,o=a?Math.round((e?.attributes?.brightness??255)/2.55):0,l=s||this._label(t),d=this._lightIcon(t),c=!!e?.attributes?.smartvanio_parent_entity_id,p=!c&&/strip/i.test(t),h=c?"Segment":p?"Strip":"";e?.attributes?.supported_color_modes?.some(t=>"rgb"===t||"hs"===t||"xy"===t);const u=e?.attributes?.rgb_color,g=i.has(t);if(this._layoutMode){const e=this._isSmartvanioLight(t);return j`
              ...
              `}const m=c?e?.attributes?.smartvanio_parent_entity_id??t:t,v=this._lpBriLocal?.eid===t?this._lpBriLocal.pct:o;let f=null,b=null;const x=this._getActivePatternName(t);if(x){b=this._getEntityPatterns(t)[x],b&&(f=this._patternGradientCSS(b))}const y=b?.length?`rgb(${b[0].r},${b[0].g},${b[0].b})`:null,_=y||(u?`rgb(${u.join(",")})`:"var(--sv-accent)"),w=n?"var(--sv-red)":a&&y?y:a&&u?`rgb(${u.join(",")})`:a?"var(--sv-accent)":"var(--sv-text-secondary)";return j`
              <div class="lp-row ${a?"on":""} ${n?"unavail":""}"
                ...
                <div class="lp-header" @click=${()=>{this._lightLpFired?this._lightLpFired=!1:n||this._toggleLight(t)}}>
                  <ha-icon class="lp-icon" icon="${d}"
                    style="--mdc-icon-size:20px; opacity:${n?.25:a?1:.55}; color:${w}"></ha-icon>
                  <div class="lp-info">
                    <span class="lp-name"><span class="lp-name-text">${l}</span>${h?j`<span class="lp-type">${h}</span>`:""}</span>
                    <span class="lp-bri">${n?"Unavailable":a?v+"%":"Off"}</span>
                  </div>
                ...
                ${n?"":j`
                  <div class="lp-slider-wrap" style="--lp-bri:${v}; --lp-color:${_}"
                    @pointerdown=${e=>this._lpBriPick(e,t,!0)}
                    @pointermove=${e=>this._lpBriPick(e,t,!1)}
                    @pointerup=${e=>this._lpBriRelease(e,t)}>
                    <div class="lp-slider-track"><div class="lp-slider-fill" style="${f?`background:${f}; width:100%; opacity:0.35`:""}"></div></div>
                    <div class="lp-slider-thumb" style="${f?"background:none; border-color:rgba(255,255,255,0.8)":""}"></div>
                  </div>
                `}
              </div>
            `})}
      ...
      <div class="panel-climate">
        ...
        >
          ...
          <path
            ...
            stroke-dasharray="${M.toFixed(1)} ${(Wn+20).toFixed(1)}"
